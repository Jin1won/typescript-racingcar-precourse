...
- n개의 자동차 이름과 전진 횟수를 입력받을 인풋창과 각각의 정보를 받아와서 제출할 버튼이 필요하다.

  - 자동차 이름은 쉼표(,)를 기준으로 구분한다.

  - 각 자동차의 이름이 5자 이하인지 체크한다.

  - 만약 사용자가 잘못된 입력 값을 작성한 경우 alert을 이용해 메시지를 보여주고, 다시 입력할 수 있게 한다.

- 입력한 이름으로 자동차 객체들을 생성한다.

- n개의 무작위 숫자를 생성한다.

  - 무작위 숫자가 4이상인지 판별한 뒤, 4 이상이라면 해당 자동차를 전진시킨다.

- 각 시도의 전진 결과를 보여준다.

  - 전진 결과는 누적해서 더해준다.

- 누적 결과가 가장 높은 자동차들을 최종 우승자로 보여준다.
